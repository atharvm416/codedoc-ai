...
def _partial_files_from_meta(
    ...
    """Parse `partial_files` into `(schema-2 tree states, legacy v1 rel_paths)`.
    ...
    for rel_path in sorted(raw):
        ...
        for node_id, node_value in raw_nodes.items():
            ...
            try:
                node = ReductionNodeState(
                    ...
                    rel_path=node_value.get("rel_path", normalized_rel_path),
                    content_hash=node_value.get("content_hash", ""),
                    division_plan_digest=node_value.get("division_plan_digest", ""),
                    reduction_tree_digest=node_value.get("reduction_tree_digest", ""),
        ...
        try:
            partial_header = SplitTreeState(
                ...
                rel_path=value.get("rel_path", normalized_rel_path),
...
def _validate_last_run(path: Path, last_run: object) -> None:
    ...
    present_split_fields = _LAST_RUN_SPLIT_INTEGER_FIELDS & set(last_run)
