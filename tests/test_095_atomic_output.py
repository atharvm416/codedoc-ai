"""0.9.5 — atomic completed-output writing.

Covers the canonical ``atomic_write_text`` helper (UTF-8, unique temp sibling,
stale-temp noninterference, no in-place truncation, OSError propagation) and the
``both``-mode Markdown-first / JSON-last finalization with failure injected at
each replacement position.
"""
...
# ---------------------------------------------------------------------------
# both-mode finalization ordering and failure injection
# ---------------------------------------------------------------------------
