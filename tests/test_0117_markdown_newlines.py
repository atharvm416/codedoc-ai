"""0.11.7 — CRLF / bare-CR compatible legacy Markdown reading.

In-memory regression proof for the reader newline normalization.  LF, CRLF, and
bare-CR representations of the same CodeDoc Markdown must parse to an identical
view and identical ownership/metadata, with no disk mutation, and without
weakening any fail-closed behaviour.

Every variant is built in memory from a canonical LF string and written with
``Path.write_bytes`` — never ``write_text`` — so the on-disk bytes are exactly
the intended newline form.  ``write_text`` defaults to ``newline=None``, which on
Windows retranslates ``\\n`` to ``\\r\\n`` and would turn the CRLF variant into
``\\r\\r\\n`` and the bare-CR variant into CRLF, exercising inputs nobody
produces (see PLAN_0.11.7.md, "Two traps").

The legacy visible fixture deliberately carries ``schema_version`` in its
``<!-- codedoc-ai: {...} -->`` comment: a schema-less, visible-only document is
treated as versionless and requires a complete ownership envelope (metadata
comment plus embedded view), so it would raise ``ConfigError`` regardless of line
endings.
"""
...
# ---------------------------------------------------------------------------
# Canonical LF documents (built once, re-expressed per newline variant).
# ---------------------------------------------------------------------------
...
# Metadata comment present but with no schema_version and no embedded view:
# a versionless, visible-only document that must fail closed on every platform.
...
# No CodeDoc marker of any kind — a foreign Markdown file.
FOREIGN_LF = (
    "# Just some notes\n"
    "\n"
...
# Metadata comment plus a present-but-corrupt embedded base64 block.
CORRUPT_EMBEDDED_LF = (
    '<!-- codedoc-ai: {"entry_file": "main.py", "schema_version": "1.4"} -->\n'
    "# codedoc project documentation\n"
    "\n"
...
# ---------------------------------------------------------------------------
# Equivalence: the fix.  CRLF and CR must match LF (these fail before the fix).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Embedded view: already newline-tolerant; normalization must not touch payload.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Security: normalization must not broaden ownership recognition.
# ---------------------------------------------------------------------------
