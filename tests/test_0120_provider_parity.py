"""0.12.0 — OpenAI / Anthropic / Gemini parity for extension-scoped profiles.

Exercises the real provider adapters through network-free fake SDK clients (no
``create_provider`` monkeypatch), proving that the same resolved file bundle
produces identical effective requested-shape text, review-before-documentation
ordering, matching provider-call counts, and identical fail-closed behaviour on
every provider.
"""
...
def test_too_risky_blocks_documentation_on_every_provider(tmp_path, monkeypatch):
    for provider, installer in _PROVIDERS:
        ...
        # (rec is local to _run; re-run to inspect call counts.)
